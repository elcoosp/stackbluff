use sbdc_service::{init, scaffold, ingest, build_prompts, generate, clean};
use sbdc_entity::{clan, character, deck, deck_narrative_arc, generated_prompt};
use sea_orm::{Database, DatabaseConnection, ConnectionTrait, EntityTrait, ColumnTrait, QueryFilter};
...
async fn full_pipeline_e2e() {
    ...
    tokio::fs::write(&json_path, serde_json::to_string(&ingest_payload).unwrap()).await.unwrap();
    ...
    assert!(ks_full.final_positive.contains("stern king"), "Missing character injection");
    assert!(ks_full.final_positive.contains("pure white background"), "Missing background invariant");

    // Verify number card has narrative arc
    let scene_2s = prompts.iter().find(|p| p.target_card == "2s" && p.target_layer == "env").unwrap();
    assert!(scene_2s.final_positive.contains("Breach the wall"), "Missing narrative arc injection");

    // Generate (takes=0 to avoid Node call in test)
    generate::run_generate(&db, dir.path(), "e2e-deck", 0, "5-15").await.unwrap();
