...
pub async fn run_build_prompts(db: &sea_orm::DatabaseConnection, deck_id: &str) -> Result<()> {
    ...
    let junction_map: HashMap<String, junction_type::Model> = junctions.into_iter()
        .map(|j| (j.junction_id.clone(), j)).collect();
    let junction = junction_map.get(&deck_model.junction_type)
    ...
    let pattern = patterns.first().ok_or_else(|| SbdcError::Validation("No creative pattern".into()))?;

    let framings = framing_instruction::Entity::find().all(db).await?;
    let framing = framings.first().ok_or_else(|| SbdcError::Validation("No framing instruction".into()))?;

    let viralities = virality_mechanic::Entity::find().all(db).await?;
    let virality = viralities.first().ok_or_else(|| SbdcError::Validation("No virality mechanic".into()))?;

    // 4. Clans and Characters maps
    let all_clans = clan::Entity::find().all(db).await?;
    let clan_map: HashMap<String, clan::Model> = all_clans.into_iter()
        .map(|c| (c.clan_id.clone(), c)).collect();

    let all_chars = character::Entity::find().all(db).await?;
    let char_map: HashMap<String, character::Model> = all_chars.into_iter()
        .map(|c| (c.clan_id.clone(), c)).collect();
    ...
    let lore_entries = lore_entry::Entity::find()
        ...
        .all(db).await?;
    let lore_injection: String = lore_entries.iter()
    ...
    let arcs = deck_narrative_arc::Entity::find()
        ...
        .all(db).await?;
    let arc_map: HashMap<(String, String), deck_narrative_arc::Model> = arcs.into_iter()
    ...
    for prompt in &prompts {
        ...
        let clan = clan_map.get(&clan_id).ok_or_else(|| SbdcError::Validation(format!("Clan {} not found", clan_id)))?;
        let character_opt = char_map.get(&clan_id);
        let is_face = matches!(rank.as_str(), "J" | "Q" | "K" | "A");

        let arc_desc = arc_map.get(&(rank.clone(), suit.clone()))
            .map(|a| a.description.as_str())
            .unwrap_or("");

        let positive = if is_face {
            let char_desc = character_opt.map(|c| c.bust_prompt_description.as_str()).unwrap_or("majestic figure");
            let artifact_desc = character_opt.map(|c| c.artifact_default_desc.as_str()).unwrap_or("holding artifact");
            format!(
                "{} {} {} {} {} {} {} {} {} {} {} {} {}",
