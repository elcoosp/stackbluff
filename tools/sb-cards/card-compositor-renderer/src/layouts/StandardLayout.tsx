...
export const StandardLayout: React.FC<Props> = ({
  ...
  const backPadding = 8;
  const artStyle = isBack
    ? { top: backPadding, left: backPadding, width: 1000 - backPadding * 2, height: 1400 - backPadding * 2, objectFit: 'cover' as const }
    : (noPadding
      ? { top: 0, left: 0, width: 1000, height: 1400, objectFit: 'contain' as const }
      : { top: 200, left: 50, width: 900, height: 1000, objectFit: 'contain' as const });
  ...
  return (
    <div className="relative w-[1000px] h-[1400px] bg-white shadow-2xl rounded-[32px] overflow-hidden">
      ...
      <div className="absolute flex items-center justify-center pointer-events-none z-20"
        ...
        <img src={artUrl} onError={handleError(artUrl)} className={isBack ? 'w-full h-full object-cover' : 'max-w-full max-h-full object-contain'} alt="art" />
      </div>
