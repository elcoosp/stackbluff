...
def create_corner_lights():
    """
    Returns a full‑card white‑to‑transparent radial gradient.
    Improved version: draws a smaller white circle, blurs it,
    and ensures the edges are fully transparent.
    """
    tile_size = 300
    circle_radius = 140  # smaller than before to ensure fade to transparent
    blur_radius = 80
...
def composite_reversible_card(
    ...
    if center_band:
        # ✅ FIXED: center the band vertically on the exact midline
        band_y = (H // 2) - (center_band.height // 2)
        card.paste(center_band, (0, band_y), center_band)

    # LAYER 3: Corner vignette (✅ FIXED: use alpha_composite for perfect transparency)
    if corner_light:
...
def composite_standard_card(
    ...
    # LAYER 3: Corner light (✅ FIXED: alpha_composite)
