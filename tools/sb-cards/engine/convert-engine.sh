...
# Typography & Pips (Correctly Scaled for 1000x1400 + Mobile Readability)
CORNER_FONT_SIZE = 120  # Max size, will auto-shrink for long text like JOKER
PIP_SIZE_CORNER = 100   # Fallback corner pip
PIP_SIZE_PLAQUE = 90    # Below plaque pip
PIP_SIZE_LAYOUT = 160   # Center layout pips
PIP_SIZE_CENTER = 320   # Ace center feature

# Corner Plaque Config
PLAQUE_DISPLAY_SIZE = 160

# --- CUSTOM SUIT COLORS ---
COLOR_BLACK_HEX = "#1C1B1E"  # Used for Spades & Clubs
COLOR_RED_HEX = "#B82B4B"  # Used for Hearts & Diamonds

# Opacity for the edge-to-edge number template so pips remain visible
TEMPLATE_ART_OPACITY = 0.5  # 0.0 (invisible) to 1.0 (fully opaque)

# Calculate RGB tuples from Hex
...
PIP_LAYOUTS = {
    ...
    "7":  [("L", "1"), ("R", "1"), ("C", "2"), ("L", "3"), ("R", "3"), ("L", "5"), ("R", "5")],
    "8":  [("L", "1"), ("R", "1"), ("C", "2"), ("L", "3"), ("R", "3"), ("C", "4"), ("L", "5"), ("R", "5")],
    "9":  [("L", "1"), ("R", "1"), ("L", "2"), ("R", "2"), ("C", "3"), ("L", "4"), ("R", "4"), ("L", "5"), ("R", "5")],
    "10": [("L", "1"), ("R", "1"), ("C", "2"), ("L", "2"), ("R", "2"), ("L", "4"), ("R", "4"), ("C", "4"), ("L", "5"), ("R", "5")],
}
...
def load_font(size):
    try:
        ...
        pass
    try:
        return ImageFont.truetype(os.path.join(FONT_DIR, "PlayfairDisplay-Bold.ttf"), size)
    except:
        pass
    try:
...
def resolve_raw_path(filename):
    ...
    if os.path.exists(path_base):
        return path_base
    path_subfolder = os.path.join(RAW_ART_DIR, "all_background_removal_results", filename)
...
def create_corner_lights():
    """Generates a soft, feathered white radial gradient to elegantly fade out the background in the corners."""
    size = 600
    circle = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    draw = ImageDraw.Draw(circle)
    # Draw circle centered off-canvas so the brightest point is the top-left corner
    draw.ellipse([(-size, -size), (size, size)], fill=(255, 255, 255, 140))
    # Blur it massively to create a smooth, natural vignette light
    blurred = circle.filter(ImageFilter.GaussianBlur(120))
    return blurred
...
def process_pips():
    ...
    for suit in SUITS:
        ...
        for size in [PIP_SIZE_CORNER, PIP_SIZE_PLAQUE, PIP_SIZE_LAYOUT, PIP_SIZE_CENTER]:
...
def draw_corners(card, rank, suit, corner_plaque=None):
    ...
    if corner_plaque:
        # --- PLAQUE PATH ---
        plaque_resized = corner_plaque.resize((PLAQUE_DISPLAY_SIZE, PLAQUE_DISPLAY_SIZE), Image.LANCZOS)
        pad = 10
        gap = 10
        ...
        # NO CHEAP WHITE BOXES - The soft light layer beneath handles readability

        # 1. Paste Plaque (Centered horizontally, at the top)
        plaque_x = (cg_w - PLAQUE_DISPLAY_SIZE) // 2
        plaque_y = pad
        corner_group.paste(plaque_resized, (plaque_x, plaque_y), plaque_resized)

        # 2. Draw Rank INSIDE the Plaque (Letterpress style with 2px crisp white stroke)
        ...
        # Letterpress style stroke on fallback text too
        cg_draw.text(
            (cg_w // 2, pad + text_h // 2),
            rank,
            fill=color,
    ...
    card.paste(
        corner_rot,
        (W - 35 - corner_rot.width, H - 35 - corner_rot.height),
        corner_rot,
    )
...
def composite_reversible_card(rank, suit, art_path, output_path, corner_plaque=None, border=None, corner_light=None):
    ...
    # LAYER 1: Border (At the very back)
    if border:
        card.paste(border, (0, 0), border)

    # LAYER 2: Art Illustration
    ...
    box_x1, box_y1 = 50, 50
    box_x2, box_y2 = W - 50, (H // 2) - 10
    box_w = box_x2 - box_x1
    box_h = box_y2 - box_y1

    bust_resized = fit_art_to_box(bust, box_w, box_h)
    x_offset = (W - bust_resized.width) // 2
    y_offset = (H // 2) - 10 - bust_resized.height

    card.paste(bust_resized, (x_offset, y_offset), bust_resized)
    ...
    # LAYER 3: Soft Corner Vignette (Elegantly fades the background for readability)
    if corner_light:
        card.paste(corner_light, (0, 0), corner_light)
        cl_rot = corner_light.rotate(180, expand=True)
        card.paste(cl_rot, (W - cl_rot.width, H - cl_rot.height), cl_rot)

    # LAYER 4: Pips & Corners (On the very top)
...
def composite_standard_card(
    rank, suit, art_path, output_path, is_back=False, no_padding=False, art_opacity=1.0, draw_full_pip_layout=False, corner_plaque=None, border=None, corner_light=None
):
    if not os.path.exists(art_path):
        print(f"  Missing art: {art_path}")
        return
    ...
    # LAYER 3: Soft Corner Vignette
    if corner_light and not is_back:
        card.paste(corner_light, (0, 0), corner_light)
        cl_rot = corner_light.rotate(180, expand=True)
        card.paste(cl_rot, (W - cl_rot.width, H - cl_rot.height), cl_rot)

    # LAYER 4: Pips & Corners
...
def build_deck():
    ...
    if corner_plaque:
        print("  Found themed corner plaque! Applying to indices.")
    else:
        print("  No corner plaque found. Using standard fallback indices.")

    border = load_border()
    if border:
        print("  Found themed card border! Applying overlay.")
    else:
        print("  No card border found. Using standard edge-to-edge.")

    print("  Generating soft corner vignettes for premium readability...")
    ...
    composite_standard_card(
        ...
        border=None,
        corner_light=None, # No light adjustment on back
    )
    ...
    for suit in SUITS:
        for rank in FACE_RANKS:
            ...
            composite_reversible_card(
                rank, suit, resolve_raw_path(fname), f"{OUTPUT_DIR}/{out_name}", corner_plaque=corner_plaque, border=border, corner_light=corner_light
            )
    ...
    for suit in SUITS:
        for rank in RANKS[:9]:
            ...
            is_custom_narrative = os.path.exists(specific_art)
            art_src = specific_art if is_custom_narrative else template_path
            ...
            if os.path.exists(art_src):
                composite_standard_card(
                    ...
                    out_path,
                    is_back=False,
                    no_padding=not is_custom_narrative,
                    art_opacity=TEMPLATE_ART_OPACITY if not is_custom_narrative else 1.0,
                    draw_full_pip_layout=not is_custom_narrative,
                    corner_plaque=corner_plaque,
                    border=border,
                    corner_light=corner_light,
                )
    ...
    print("Deck compositing & Kling prep complete!")
