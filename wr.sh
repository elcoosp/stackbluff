...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  ROOT CAUSE: Images are IFRAMES, not IMG tags!               ║"
echo "╠══════════════════════════════════════════════════════════════╣"
echo "║                                                              ║"
echo "║  The content script looks for '#outputAreaEl img' but the    ║"
echo "║  generated images are IFRAME elements with class             ║"
echo "║  'text-to-image-plugin-image-iframe'.                        ║"
echo "║                                                              ║"
echo "║  So:                                                         ║"
echo "║  1. The 'wait for images' check NEVER triggers               ║"
echo "║  2. No images get extracted                                  ║"
echo "║  3. The script hangs forever after clicking Generate         ║"
echo "║                                                              ║"
echo "║  Fix: Detect iframes, extract canvas/img from inside them    ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""

cat > "$EXT_DIR/src/content.ts" << 'CT_V9_J7kN3'
...
git add -A
git commit -m "fix(sbdc): detect IFRAME images instead of IMG on perchance.org

Root cause: Perchance.org/fluxgen renders generated images as IFRAME
elements (class='text-to-image-plugin-image-iframe'), not IMG tags.
The content script was looking for '#outputAreaEl img' which found
ZERO elements, so the 'wait for images' loop hung forever.

Fix:
- countOutputImages() checks iframes, then img, then .t2i-image-ctn
- waitForImagesToLoad() counts iframes appearing after Generate click
- extractImageFromIframe() tries:
  1. iframe.contentDocument → canvas.toDataURL()
  2. iframe.contentDocument → img.src
  3. Fetch iframe URL directly, parse for data URLs
- Falls back to #outputAreaEl img if no iframes found
- Submits empty takes if no images could be extracted (marks prompt done)" 2>&1 || echo "Nothing new to commit"
