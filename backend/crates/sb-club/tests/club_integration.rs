...
use sb_contracts::{ClubRepo, ClubService, ClubError, LeaderboardPage, DIVISION_SIZE};
use sb_db_repos::club_repo::ClubRepoImpl;
use sb_club::ClubServiceImpl;
use sb_shared_types::{ClubId, UserId, RequestContext};
...
async fn test_create_club() {
    ...
    let club_id = svc
        .create_club(&ctx, "Test Club", Some("https://logo.example.com/img.png"), user_id)
        .await
        .expect("create club");
...
async fn test_join_club_and_duplicate() {
    ...
    let club_id = svc.create_club(&ctx, "Join Club", None, owner_id).await.expect("create");

    let member_ctx = RequestContext { request_id: Uuid::new_v4(), user_id: Some(UserId::new(Uuid::new_v4())) };
    let member_id = member_ctx.user_id.unwrap();
    svc.join_club(&member_ctx, club_id, member_id).await.expect("join club");
...
async fn test_add_xp_not_member() {
    ...
    let ctx = test_ctx();
    let owner_id = ctx.user_id.unwrap();
    let club_id = svc.create_club(&ctx, "NoXP Club", None, owner_id).await.expect("create");
...
async fn test_add_xp_member() {
    ...
    let ctx = test_ctx();
    let owner_id = ctx.user_id.unwrap();
    let club_id = svc.create_club(&ctx, "XP Club", None, owner_id).await.expect("create");
    svc.join_club(&ctx, club_id, owner_id).await.expect("join");
    svc.add_xp(&ctx, club_id, owner_id, 100).await.expect("add xp");
}
...
async fn test_leaderboard_divisions() {
    ...
    let ctx = test_ctx();
    let owner_id = ctx.user_id.unwrap();
    let club_id = svc.create_club(&ctx, "Big Club", None, owner_id).await.expect("create");

    // Join and add XP for 600 members
    for i in 0..599 {
        let member_ctx = RequestContext { request_id: Uuid::new_v4(), user_id: Some(UserId::new(Uuid::new_v4())) };
        let member_id = member_ctx.user_id.unwrap();
        svc.join_club(&member_ctx, club_id, member_id).await.expect("join");
        svc.add_xp(&member_ctx, club_id, member_id, (599 - i) as i64 * 10).await.expect("add xp");
    }
    ...
    let div1 = svc.get_leaderboard(&ctx, club_id, 1).await.expect("get leaderboard div 1");
    ...
    let div2 = svc.get_leaderboard(&ctx, club_id, 2).await.expect("get leaderboard div 2");
