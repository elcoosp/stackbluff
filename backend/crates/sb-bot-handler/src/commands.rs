...
async fn resolve_user_with_timeout(state: &Arc<BotState>, telegram_id: &str) -> anyhow::Result<UserId> {
    timeout(SERVICE_TIMEOUT, state.user_resolution.resolve_telegram_user(telegram_id))
        .await
...
async fn send_telegram_message_with_timeout(
    ...
    timeout(SERVICE_TIMEOUT, state.notification_service.send_telegram_message(chat_id, text, keyboard))
        .await
...
pub async fn handle_poker_command(
    ...
    let telegram_user_id = match message.from.as_ref().map(|u| u.id.0.to_string()) {
        ...
        None => {
            ...
            let _ = send_telegram_message_with_timeout(state, chat_id.0, "❌ Could not identify sender.".to_string(), None).await;
            return;
        }
    };
    ...
    let user_id = match resolve_user_with_timeout(state, &telegram_user_id).await {
        ...
        Err(e) => {
            ...
            let _ = send_telegram_message_with_timeout(state, chat_id.0, "❌ Could not identify you. Please start the bot in private first.".to_string(), None).await;
            return;
        }
    };
...
pub async fn handle_challenge_command(
    ...
    if challenged_telegram.is_none() {
        let _ = send_telegram_message_with_timeout(state, chat_id.0, "Usage: /challenge @username".to_string(), None).await;
        return;
    }

    let challenger_id = match resolve_user_with_timeout(state, &challenger_telegram.unwrap_or_default()).await {
        Ok(uid) => uid,
        Err(e) => {
            error!(request_id = %ctx.request_id, error = %e, "Challenger resolve failed");
            let _ = send_telegram_message_with_timeout(state, chat_id.0, "❌ Could not identify you.".to_string(), None).await;
            return;
        }
    };
    let challenged_id = match resolve_user_with_timeout(state, challenged_telegram.as_ref().unwrap()).await {
        ...
        Err(e) => {
            ...
            let _ = send_telegram_message_with_timeout(state, chat_id.0, format!("❌ User @{} not found.", challenged_telegram.unwrap()), None).await;
            return;
        }
    };
