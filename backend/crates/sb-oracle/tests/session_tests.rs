...
impl UserRepo for DummyRepo {
    async fn create_user(&self, _ctx: RequestContext, _create: sb_contracts::repo_api::UserCreate) -> Result<UserId, PersistenceError> { unimplemented!() }
    ...
    async fn has_active_season_pass(&self, _ctx: RequestContext, _user_id: UserId) -> Result<bool, PersistenceError> { Ok(false) }
}
...
async fn remaining_zero_after_max() {
    let session = SessionManager::new();
    let user_id = UserId::new(uuid::Uuid::new_v4());
    for _ in 0..5 { session.try_consume(user_id).await; }
    let remaining = session.remaining(user_id).await;
