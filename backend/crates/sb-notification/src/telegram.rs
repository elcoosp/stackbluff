...
impl TelegramSender {
    pub fn new(bot_token: String) -> Self {
        Self { bot_token, http_client: Client::new() }
    }

    pub async fn send(&self, _user_id: UserId, event: &NotificationEvent) -> Result<(), AppError> {
        let text = match event {
            NotificationEvent::TournamentReminder { tournament_name, start_time } =>
                format!("🏟️ Tournament '{}' starts at {}", tournament_name, start_time),
            NotificationEvent::StreakAlert { streak_count } =>
                format!("🔥 You're on a {}-day streak!", streak_count),
            NotificationEvent::ReferralBonus { from_user_id, amount } =>
