//! Game state for a single poker hand: blinds, dealing, betting rounds, showdown.
...
pub enum ActionError {
    ...
    AlreadyAllIn,
    #[error("Invalid raise amount (must be at least min raise)")]
    InvalidRaise,
...
impl GameState {
    pub fn new_hand(mut players: Vec<PlayerId>, blinds: (ChipAmount, ChipAmount)) -> HandId {
        ...
        let mut player_states: Vec<PlayerHandState> = players
            ...
            .map(|pid| PlayerHandState {
                player_id: pid,
                hole_cards: None,
                bet_this_round: ChipAmount(0),
                total_bet: ChipAmount(0),
                is_all_in: false,
                has_folded: false,
            })
            .collect();
        ...
        let mut round_bets = vec![ChipAmount(0); player_states.len()];
    ...
    pub fn apply_action(&mut self, player_id: PlayerId, action: Action) -> Result<(), ActionError> {
        ...
        match action {
            ...
            Action::Raise(raise_amount) => {
                ...
                if total_bet < self.smallest_bet + self.min_raise {
                    return Err(ActionError::InvalidRaise);
                }
                let additional = raise_amount;
                self.add_bet(idx, additional);
    ...
    fn add_bet(&mut self, player_idx: usize, amount: ChipAmount) {
        let player = &mut self.players[player_idx];
        player.bet_this_round = player.bet_this_round + amount;
        player.total_bet = player.total_bet + amount;
        self.round_bets[player_idx] = self.round_bets[player_idx] + amount;
    ...
    fn reset_round(&mut self) {
        for i in 0..self.players.len() {
            self.players[i].bet_this_round = ChipAmount(0);
        }
        self.round_bets.fill(ChipAmount(0));
        self.smallest_bet = ChipAmount(0);
        ...
        while self.players[start].has_folded || self.players[start].is_all_in {
            ...
            if start == (self.dealer_index + 1) % self.players.len() {
                break;
            }
        }
    ...
    pub fn calculate_pot_winners(&self) -> Vec<Winner> {
        let active: Vec<usize> = self.players
            .iter()
            .enumerate()
        ...
        if active.len() == 1 {
            return vec![Winner {
                player_id: self.players[active[0]].player_id,
                amount: self.pot,
        ...
        let share = self.pot / ChipAmount(winners.len() as i64);
        winners
            .into_iter()
            .map(|pid| Winner {
                player_id: pid,
...
mod tests {
    ...
    #[test]
    fn test_new_hand_deals_cards() {
        let players = vec![PlayerId(1), PlayerId(2)];
        let hand_id = GameState::new_hand(players, (ChipAmount(5), ChipAmount(10)));
    ...
    fn test_fold_action() {
        let players = vec![PlayerId(1), PlayerId(2)];
        let mut state = GameState::new_hand(players, (ChipAmount(5), ChipAmount(10)));
        let player_to_act = state.players[state.current_player_index].player_id;
        assert!(state.apply_action(player_to_act, Action::Fold).is_ok());
        assert!(state.players[state.current_player_index].has_folded);
    }

    #[test]
    fn test_illegal_raise() {
        let players = vec![PlayerId(1), PlayerId(2)];
        let mut state = GameState::new_hand(players, (ChipAmount(5), ChipAmount(10)));
        let player_to_act = state.players[state.current_player_index].player_id;
        // raise below min raise (min raise is big blind)
        let result = state.apply_action(player_to_act, Action::Raise(ChipAmount(1)));
        assert_eq!(result, Err(ActionError::InvalidRaise));
    }
}
