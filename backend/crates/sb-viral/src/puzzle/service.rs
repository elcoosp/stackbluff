...
static SUBMISSIONS: Lazy<IntCounter> =
    Lazy::new(|| IntCounter::with_opts(Opts::new("puzzle_submissions_total", "Total")).unwrap());
static CORRECT: Lazy<IntCounter> =
    Lazy::new(|| IntCounter::with_opts(Opts::new("puzzle_correct_total", "Correct")).unwrap());
...
pub async fn submit_puzzle_action(
    ...
    if let Some(ex) = repo
        .find_submission(user_id, today)
        .await
        .map_err(|e| PuzzleServiceError::Db(e.to_string()))?
    {
        return Err(PuzzleServiceError::AlreadySubmitted {
            correct: ex.is_correct,
            action: ex.selected_action.to_string(),
        });
    }

    let puzzle = data::get_today_puzzle().ok_or(PuzzleServiceError::NoPuzzle)?;
    let action: PuzzleAction = req
        .selected_action
        .parse()
        .map_err(|_| PuzzleServiceError::InvalidAction)?;

    if !puzzle
        .possible_actions
        .iter()
        .any(|a| a.to_lowercase() == action.to_string())
    {
        return Err(PuzzleServiceError::InvalidAction);
    }

    let is_correct = action.to_string() == puzzle.correct_action.to_lowercase();
    let record = PuzzleSubmissionRecord {
        user_id,
        puzzle_date: today,
...
pub enum PuzzleServiceError {
    ...
    AlreadySubmitted { correct: bool, action: String },
    #[error("No puzzle")]
    NoPuzzle,
