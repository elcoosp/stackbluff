...
interface GameState {
  tableId: string | null; seats: Record<number, Seat>; communityCards: Card[]; pot: number;
  sidePots: Array<{ amount: number; eligibleSeats: number[] }>;
  currentRound: 'preflop' | 'flop' | 'turn' | 'river' | 'showdown'; heroSeat: number | null;
  heroHoleCards: [Card, Card] | null; actionRequired: boolean; minRaise: number; maxRaise: number;
  toCall: number; timeRemainingMs: number | null; winners: Winner[] | null;
  setSnapshot: (snapshot: TableState) => void; setHeroHoleCards: (cards: [Card, Card]) => void;
  applyActionBroadcast: (broadcast: ActionBroadcast) => void; setActionRequired: (required: ActionRequired) => void;
  clearActionRequired: () => void; setHandResult: (result: HandResult) => void; reset: () => void;
}
export const useGameStore = create<GameState>((set) => ({
  tableId: null, seats: {}, communityCards: [], pot: 0, sidePots: [], currentRound: 'preflop', heroSeat: null,
  heroHoleCards: null, actionRequired: false, minRaise: 0, maxRaise: 0, toCall: 0, timeRemainingMs: null, winners: null,
