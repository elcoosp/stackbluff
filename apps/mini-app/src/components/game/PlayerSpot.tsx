...
export const PlayerSpot = ({ seat, isHero }: { seat: any; isHero?: boolean }) => (
  <div className="flex flex-col items-center gap-1 p-2 rounded-xl glass-hub min-w-[80px]">
    <PlayerAvatar name={seat.display_name} avatarUrl={seat.avatar_url} isActive={seat.is_active} />
    <div className="flex gap-1">
      {isHero ? (seat.hole_cards?.map((c, i) => <Card key={i} rank={c.rank} suit={c.suit} />)) : (<><CardBack /><CardBack /></>)}
    </div>
