...
// Positions derived from the high-fidelity mockup (9-max)
const desktopPositions: Record<number, { top: string; left: string; transform: string }> = {
  0: { top: '82%', left: '5%', transform: 'translate(-50%, -50%)' },   // Seat 1
  1: { top: '40%', left: '2%', transform: 'translate(-50%, -50%)' },    // Seat 2
  2: { top: '10%', left: '15%', transform: 'translate(-50%, -50%)' },   // Seat 3
  3: { top: '0%', left: '50%', transform: 'translate(-50%, -50%)' },    // Seat 4
  4: { top: '10%', right: '15%', transform: 'translate(50%, -50%)' },   // Seat 5
  5: { top: '40%', right: '2%', transform: 'translate(50%, -50%)' },    // Seat 6
  6: { top: '82%', right: '5%', transform: 'translate(50%, -50%)' },    // Seat 7
  7: { bottom: '-45px', left: '50%', transform: 'translateX(-50%)' },   // Hero (seat 8)
  8: { top: '92%', left: '28%', transform: 'translate(-50%, -50%)' },   // Seat 9
};
...
export const SeatGrid = ({ seats, heroSeat, isDesktop }: { seats: Record<number, any>; heroSeat: number; isDesktop: boolean }) => (
  <div className="relative w-full h-full">
    {Object.values(seats).map((seat) => {
      ...
      if (!pos) return null;
      return (
        <motion.div key={seat.seat_index} layout className="absolute" style={{ ...pos, zIndex: seat.seat_index === heroSeat ? 350 : 50 }}>
