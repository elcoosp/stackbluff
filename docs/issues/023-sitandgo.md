...
Implement Sit & Go tournament support (REQ-FUNC-018):

- `POST /tournaments/sit-and-go` with `max_players` (6 or 9) and `buy_in` chips.
...
- Eliminated players receive a notification and can leave or spectate.
- Tournament table uses the same WebSocket protocol as cash games, plus `tournament.elimination` and `tournament.result` messages.

## Acceptance criteria

- [ ] Creating a 6‑player S&G costs buy_in chips; when 6th player joins, game starts.
- [ ] Blind levels increase every 10 minutes (configurable).
- [ ] Winner receives correct prize pool distribution.
- [ ] Integration test: 6 players register, tournament completes, payouts correct.

## Blocked by

#001 (contracts – TournamentApi), #009 (table actor loop exists)
