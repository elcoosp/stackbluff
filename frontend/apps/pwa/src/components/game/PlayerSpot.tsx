...
const getActionStyles = (text?: string) => {
  switch (text) {
    case 'FOLD':
      return { Icon: LogOut, color: 'rgba(248, 113, 113, 1)' };
    case 'CHECK':
      return { Icon: Check, color: 'rgba(78, 222, 163, 1)' };
    case 'CALL':
      return { Icon: DollarSign, color: 'rgba(78, 222, 163, 1)' };
    case 'BET':
    case 'RAISE':
      return { Icon: TrendingUp, color: 'rgba(255, 255, 255, 0.85)' };
    case 'ALL-IN':
      return { Icon: Swords, color: 'rgba(251, 191, 36, 1)' };
    default:
      return { Icon: null, color: 'rgba(78, 222, 163, 1)' };
  }
};
...
const CARD_BACKFACE_STYLE: CSSProperties = {
...
const CardGroup = memo(
  ({
    ...
    const renderCard2 =
      hole_cards && hole_cards.length > 1 ? (
        <AnimatePresence mode="popLayout">
          {showCardsFaceUp ? (
            <motion.div
              key="front2"
...
export const PlayerSpot = memo(
  ({
    ...
    const dealerButton = isDealer && (
      <motion.div
        ...
        className={cn(
          ...
          isMobile
            ? isLeftSide
              ? '-bottom-3 -right-4 w-5 h-5 text-[8px]'
              : '-bottom-3 -left-4 w-5 h-5 text-[8px]'
            : isLeftSide
    ...
    const formattedStack =
      safeStack >= 1000
        ? `$${(safeStack / 1000).toFixed(safeStack % 1000 === 0 ? 0 : 1)}k`
        : `$${safeStack}`;
    ...
    const statsElement = useMemo(() => {
      ...
      return (
        <div
          className={cn(
            'flex items-baseline h-[8px] overflow-hidden leading-none gap-[2px]',
            baseStyle,
            textSize,
          )}
        >
          ...
          <span
            className={cn(
              'shrink-0 inline-block min-w-[14px] text-right tabular-nums transition-colors duration-500',
              flashColors.vpip || 'text-on-surface-variant',
            )}
          >
            {vpipDisplay}
          </span>
          ...
          <span
            className={cn(
              'shrink-0 inline-block min-w-[14px] text-right tabular-nums transition-colors duration-500',
              flashColors.pfr || 'text-on-surface-variant',
            )}
          >
            {pfrDisplay}
          </span>
          ...
          <span
            className={cn(
              'shrink-0 inline-block min-w-[18px] text-right tabular-nums transition-colors duration-500',
              flashColors.af || 'text-on-surface-variant',
            )}
          >
            {afDisplay}
          </span>
    ...
    }, [animatedVpip, animatedPfr, animatedAf, flashColors, isMobile, stats]);
    ...
    const allInGlow =
      is_all_in && !is_folded ? (
        <motion.div
          ...
          style={{ boxShadow: '0 0 15px rgba(239,68,68,0.3), 0 0 30px rgba(239,68,68,0.15)' }}
        />
      ) : null;
          ...
          <AnimatePresence mode="wait">
            {showActionBadge && (
              ...
              >
                ...
                {!isFolded && current_bet > 0 && (
                  <span className="opacity-80 font-bold">${current_bet}</span>
                )}
              </motion.div>
            )}
          </AnimatePresence>
