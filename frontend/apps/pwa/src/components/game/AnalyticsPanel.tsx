...
export const TacticalOracle = ({ winProb, potOdds }: { winProb: number; potOdds: number }) => {
          ...
          {expanded && (
            ...
            >
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-mono font-bold text-tertiary tabular-nums">
                  {displayWinProb}
                </span>
                <span className="text-xs font-mono text-tertiary/60">
                  <Trans>win%</Trans>
                </span>
              </div>
              <div className="h-px bg-gradient-to-r from-transparent via-white/10 to-transparent origin-left" />
              <div className="flex items-baseline gap-2">
                <span className="text-lg font-mono font-bold text-on-surface tabular-nums">
                  {displayPotOdds}
                </span>
...
export const HandStrength = ({ bestHand, strength }: { bestHand: string; strength: number }) => {
  const [expanded, setExpanded] = useState(true);
  ...
  const strengthColor =
    displayStrength >= 70
      ...
      : displayStrength >= 40
        ...
        : 'text-red-400';
  const barColor =
    displayStrength >= 70 ? 'bg-tertiary' : displayStrength >= 40 ? 'bg-amber-400' : 'bg-red-400';
            ...
            >
              ...
              <div className="space-y-1">
                <div className="flex items-baseline justify-between">
                  <span className={`text-2xl font-mono font-bold tabular-nums ${strengthColor}`}>
                    {displayStrength}
                  </span>
                  <span className="text-xs font-mono text-on-surface-variant">
                    <Trans>/ 100</Trans>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="text-[9px] font-mono tracking-[0.15em] text-on-surface-variant uppercase">
            <Trans>Win%</Trans>
          </div>
          <div className="text-sm font-mono font-bold text-tertiary tabular-nums">
            {displayWinProb}%
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="text-sm font-mono font-bold text-on-surface tabular-nums">
            {displayPotOdds}:1
          </div>
        </motion.div>
        <div className="h-5 w-px bg-white/10" />
        ...
        >
          ...
          <div className={`text-sm font-mono font-bold tabular-nums ${strengthColor}`}>
            {displayStrength}
          </div>
        </motion.div>
      </div>
