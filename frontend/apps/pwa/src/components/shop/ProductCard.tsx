...
import { Coins, Crown, ShieldCheck, Sparkles } from 'lucide-react';
...
export function ProductCard({ product, onPurchase, className }: ProductCardProps) {
  ...
  const _isClubPro = product.type === 'club_pro';
  ...
  const badgeText = isChips ? t`Chips` : isSeasonPass ? t`Season Pass` : t`Club Pro`;
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full min-h-[340px] p-6">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center">
          ...
          <Badge
            variant="outline"
            className="border-tertiary/40 text-tertiary text-[10px] font-label-caps uppercase tracking-widest backdrop-blur-md bg-black/30"
          >
            {badgeText}
          </Badge>
        </div>
